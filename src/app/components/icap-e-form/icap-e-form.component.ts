...
@Component({
  selector: 'app-icap-e-form',
  templateUrl: './icap-e-form.component.html',
  styleUrls: ['./icap-e-form.component.scss'],
})
export class IcapEFormComponent implements OnInit {

  private icapEForm: FormGroup;
  ...
  constructor(private route: Router, private formBuilder: FormBuilder, private stockRolesService: StockRolesService, private userService: UserService) {
    this.icapEForm = this.formBuilder.group({
  ...
  get form() { return this.icapEForm.controls; }
  ...
  createUser = () => {
    ...
    let userData = this.icapEForm.getRawValue()
  ...
  getUserInformation = (id) => {
    this.userService.getNames({ id: id }).subscribe((res: any) => {
      ...
      this.icapEForm.patchValue({name: name, first_last_name: first_last_name, second_last_name: second_last_name})
    })
  }
