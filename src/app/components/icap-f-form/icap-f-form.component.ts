...
@Component({
  selector: 'app-icap-f-form',
  templateUrl: './icap-f-form.component.html',
  styleUrls: ['./icap-f-form.component.scss'],
})
export class IcapFFormComponent implements OnInit {

  private icapFForm: FormGroup;
  ...
  constructor(private route: Router, private formBuilder: FormBuilder, private stockRolesService: StockRolesService, private userService: UserService) {
    this.icapFForm = this.formBuilder.group({
  ...
  get form() { return this.icapFForm.controls; }
  ...
  createUser = () => {
    ...
    let userData = this.icapFForm.getRawValue()
  ...
  getUserInformation = (id) => {
    this.userService.getNames({ id: id }).subscribe((res: any) => {
      ...
      this.icapFForm.patchValue({name: name, first_last_name: first_last_name, second_last_name: second_last_name})
    })
  }
