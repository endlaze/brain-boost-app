...
@Component({
  selector: 'app-icap-d-form',
  templateUrl: './icap-d-form.component.html',
  styleUrls: ['./icap-d-form.component.scss'],
})
export class IcapDFormComponent implements OnInit {

  private icapDForm: FormGroup;
  ...
  constructor(private route: Router, private formBuilder: FormBuilder, private stockRolesService: StockRolesService, private userService: UserService) {
    this.icapDForm = this.formBuilder.group({
  ...
  get form() { return this.icapDForm.controls; }
  ...
  createUser = () => {
    ...
    let userData = this.icapDForm.getRawValue()
  ...
  getUserInformation = (id) => {
    this.userService.getNames({ id: id }).subscribe((res: any) => {
      ...
      this.icapDForm.patchValue({name: name, first_last_name: first_last_name, second_last_name: second_last_name})
    })
  }
