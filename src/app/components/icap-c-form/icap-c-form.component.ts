...
@Component({
  selector: 'app-icap-c-form',
  templateUrl: './icap-c-form.component.html',
  styleUrls: ['./icap-c-form.component.scss'],
})
export class ICAPCFormComponent implements OnInit {

  private icapCForm: FormGroup;
  ...
  constructor(private route: Router, private formBuilder: FormBuilder, private stockRolesService: StockRolesService, private userService: UserService) {
    this.icapCForm = this.formBuilder.group({
  ...
  get form() { return this.icapCForm.controls; }
  ...
  createUser = () => {
    ...
    let userData = this.icapCForm.getRawValue()
  ...
  getUserInformation = (id) => {
    this.userService.getNames({ id: id }).subscribe((res: any) => {
      ...
      this.icapCForm.patchValue({name: name, first_last_name: first_last_name, second_last_name: second_last_name})
    })
  }
